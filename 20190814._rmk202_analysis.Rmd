...
In these section I aim to identify and extract all CRISPR spacers in all genomes (Dialact+NCBI) of the species present on Dialact.
...
now we try to see if there are differences in spacers between the 
...
##==================
  ...
  ##Streptococcus thermophilus
  #--------------
  rm -r $mainPath/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/
  mkdir -p $mainPath/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/
  
 # rm $mainPath/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/GenomePath.txt
  
  cat /archiv/Projects/2018_Culturomics/06_Spades/S72/PilerCR/${date}/S72.pilarCR_out_final /archiv/Projects/2018_Culturomics/06_Spades/S50/PilerCR/${date}/S50.pilarCR_out_final >> \
  $mainPath/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/GenomePath.txt
  
   cd-hit -i $mainPath/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/GenomePath.txt \
    -o $mainPath/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/results.txt -c 0.8 -M 30000 -T ${threads}
  ...
  rm /archiv/Projects/2018_Culturomics/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/clustering_results.txt
  
  for ((i=start; i<=end; i++))
  do
     echo "number: $i"
     
      grep "^${i}" -c /archiv/Projects/2018_Culturomics/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/results.txt.clstr >> \
      /archiv/Projects/2018_Culturomics/12_CRISPR_isolates/cdhit/${date}/Streptococcus_thermophilus/clustering_results.txt
...
#Lactobacillus delbrueckii
#--------------
rm -r $mainPath/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/
mkdir -p $mainPath/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/

rm $mainPath/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/GenomePath.txt

for id in $(ls /archiv/Projects/2018_Culturomics/06_Spades/ |grep "^L")
do

cat /archiv/Projects/2018_Culturomics/06_Spades/${id}/PilerCR/20190203/${id}.pilarCR_out_final | sed -e "s/>/>${id}_/g"  >> \
$mainPath/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/GenomePath.txt

done

grep "^>" -c $mainPath/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/GenomePath.txt



 cd-hit -i $mainPath/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/GenomePath.txt \
  -o $mainPath/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/results.txt -c 0.7 -M 30000 -T ${threads}
...
do
   echo "number: $i"
    ...
    grep "^${i}" -c /archiv/Projects/2018_Culturomics/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/results.txt.clstr >> \
    /archiv/Projects/2018_Culturomics/12_CRISPR_isolates/cdhit/${date}/Lactobacillus_delbrueckii/clustering_results.txt
...
mkdir -p /archiv/Projects/2019_RMK202_analysis/03_mapping2Nanopre/02_againstpolished_single_K1_final_kneaddata/freebayesOuput
    ...
    freebayes -F 0.05 -C 2 --min-coverage 20 -p 1 \
      ...
      /archiv/Projects/2019_RMK202_analysis/02_flye_Assembly/merged/RMK202_merged.fasta > \
      /archiv/Projects/2019_RMK202_analysis/03_mapping2Nanopre/02_againstpolished_single_K1_final_kneaddata/freebayesOuput/variantCallsfreebayes_all_f_005_min_20x.vcf
...
scp vincent@130.223.51.151:/archiv/Projects/2019_RMK202_analysis/03_mapping2Nanopre/02_againstpolished_single_K1_final_kneaddata/freebayesOuput/variantCallsfreebayes_all_f_005_min_20x.vcf /home/vincent/Desktop/Projects/2019_RMK202_analysis/06_snpCalling2ONT

grep "^##" -v /home/vincent/Desktop/Projects/2019_RMK202_analysis/06_snpCalling2ONT/variantCallsfreebayes_all_f_005_min_20x.vcf >  /home/vincent/Desktop/Projects/2019_RMK202_analysis/06_snpCalling2ONT/variantCallsfreebayes_all_preped.vcf
...
library(vcfR)
library(readr)
snps_freebayes <- read_delim("/home/vincent/Desktop/Projects/2019_RMK202_analysis/06_snpCalling2ONT/variantCallsfreebayes_all_preped.vcf", "\t", escape_double = FALSE, trim_ws = TRUE)
...
##wide2long
snps_freebayes_long <- gather(snps_freebayes, sample, Snps, "RMK202":"L44", factor_key=TRUE) 
nrow(snps_freebayes_long)

snps_freebayes_long <- gather(snps_freebayes, sample, Snps, "RMK202":"L44", factor_key=TRUE,na.rm = TRUE) 
nrow(snps_freebayes_long)
...
## ===========all samples in wide format and cleaned
sample_relative_safe_final_tsne_safe <- sample_relative_safe_final_tsne
...
##remove isolates
sample_relative_safe_woldel <- sample_relative_temp_cleaned[grep("^L\\d",sample_relative_temp_cleaned$sample,invert = TRUE),] 
...
## ===========all metagenomes in wide format 
sample_relative_safe_final$sample <- droplevels(sample_relative_safe_final$sample)


##reorder samples
sample_relative_safe_final$sample_02 = factor(sample_relative_safe_final$sample, levels=c("Lyo_202_2012","Lyo_202_2014","RMK202","Konserve_202","Versand_202"))
levels(sample_relative_safe_final$sample_02)

##rename contigs

table(sample_relative_safe_final$X.CHROM)
...
colnames(sample_relative_safe_final_tsne)

 sample_relative_safe_final_no_snps <- sample_relative_safe_final_tsne[,c("RMK202","Lyo_202_2014","S72","Lyo_202_2012","L99","L80","Versand_202","L71","L70","Konserve_202","L108","L35","S50","L39","L104","L44")]

 ##meta
sample_relative_safe_final_snps_meta <- sample_relative_safe_final_tsne[,c("Lyo_202_2012","Lyo_202_2014","RMK202","Konserve_202","Versand_202")]
sum(rowSums(sample_relative_safe_final_snps_meta,na.rm = TRUE)==0)
sample_relative_safe_final_meta_cleaned <- sample_relative_safe_final_tsne[(rowSums(sample_relative_safe_final_snps_meta,na.rm = TRUE)>0),]
# sample_relative_safe_final_tsne[(rowSums(sample_relative_safe_final_snps_meta,na.rm = TRUE)==0),]

 ##isolates
sample_relative_safe_final_snps_iso <- sample_relative_safe_final_tsne[,c("S72","L99","L80","L71","L70","L108","L35","S50","L39","L104","L44")]
sum(rowSums(sample_relative_safe_final_snps_iso,na.rm = TRUE)==0)
sample_relative_safe_final_iso_cleaned <- sample_relative_safe_final_tsne[(rowSums(sample_relative_safe_final_snps_iso,na.rm = TRUE)>0),]
# sample_relative_safe_final_tsne[(rowSums(sample_relative_safe_final_snps_iso,na.rm = TRUE)==0),]

###---venn diagramm
table(sample_relative_safe_final_iso_cleaned$X.CHROM)
##------Sterm

onlyfirst_sterm <- sample_relative_safe_final_iso_cleaned[sample_relative_safe_final_iso_cleaned$X.CHROM=="S_thermophilus_RMK202","site"]
onlysecond_sterm <- sample_relative_safe_final_meta_cleaned[sample_relative_safe_final_meta_cleaned$X.CHROM=="S_thermophilus_RMK202","site"]

both_n <- sum(onlysecond_sterm %in% onlyfirst_sterm)
first_n <- length(onlyfirst_sterm)
second_n <- length(onlysecond_sterm)
13186+1849+386
1849/15421
13186/15421
386/15421
nrow(sample_relative_safe_final_meta_cleaned)
venn.plot <- draw.pairwise.venn(first_n,second_n, both_n, c("in Metagenome", "in Isolates"))
...
#venn(list(isolates = onlyfirst_sterm, meta = onlysecond_sterm))

##------Ldel

onlyfirst_sterm <- sample_relative_safe_final_iso_cleaned[sample_relative_safe_final_iso_cleaned$X.CHROM=="L_delbrueckii_RMK202","site"]
onlysecond_sterm <- sample_relative_safe_final_meta_cleaned[sample_relative_safe_final_meta_cleaned$X.CHROM=="L_delbrueckii_RMK202","site"]
...
table(sample_relative_safe_final_tsne_safe$X.CHROM)
dim(sample_relative_safe_final_tsne_safe)
length(unique(sample_relative_safe_final_tsne_safe$site))


tsne_prep <- sample_relative_safe_final_tsne_safe[,c("Lyo_202_2012","Lyo_202_2014","RMK202","Konserve_202","Versand_202")]

(rowSums(tsne_prep,na.rm = TRUE)==0)
...
tsne <- Rtsne(sample_relative_safe_final_tsne_safe_sterm[,c("Lyo_202_2012","Lyo_202_2014","RMK202","Konserve_202","Versand_202")], dims = 2, perplexity=30, verbose=TRUE, max_iter = 500)
...
100*(nrow(not_explained_ldel_plot_final)/nrow(sample_relative_safe_final_tsne_safe_ldel))
...
dim(not_explained_ldel_plot_final)
  p1 <- ggplot(not_explained_ldel_plot_final,aes(x=sample_02,y=allel_frequency,group=site,color=grouping))+ geom_line(size=0.2, alpha=.1)+ 
   ...
   labs("",
         x="",
         y="alternative allel frequency",
          title=paste0("NOT-explained by isolates ",round(100*(nrow(not_explained_ldel_plot_final)/nrow(sample_relative_safe_final_tsne_safe_ldel)),digits = 2)))+
  theme_classic()+
 theme(#axis.text.x = element_text(angle = 75, hjust = 1,size=9),
       axis.text.x=element_blank(),
          rect = element_rect(fill = "transparent"), # all rectangles      #axis.text.x = element_blank(),
...
/home/vincent/miniconda2/bin/bp_genbank2gff3.pl /archiv/Projects/2019_pilotplant/02_annotation/rastServer/Streptococcus_phage.fasta.gbk
...
rm -r /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/phageDBmapping/
...
echo ${num}"/16  :" ${names}
    num=$((num+1))
    rm -r /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/phageDBmapping/${names}
  
    mkdir -p /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/phageDBmapping/${names}
    

bedtools coverage -bed -a /archiv/Projects/2019_pilotplant/02_annotation/rastServer/all_phages.bed -b /archiv/Projects/2019_RMK202_analysis/03_mapping2Nanopre/02_againstpolished_single_K1_final_kneaddata/${names}/bwaMapping2DB/${names}_rawIllumina_bwamem_sorted.bam  > \
  /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/phageDBmapping/${names}/${names}2phageDB.bed 
...
names=Konserve_202
 #rm -r /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/phageDBmapping
 mkdir -p /home/vincent/Projects/2019_pilotplant/05_mapping2DB/phageDBmapping/finalmapping/
for names in $(cat /archiv/Projects/2019_RMK202_analysis/01_log/names_onlymeta.txt)
    do
    
echo $name 
  
    cat /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/phageDBmapping/${names}/${names}2phageDB.bed | sed -e 's/ID=.*;OG=/OG=/g'|awk -F "[\t]" -v namess="$names"  'BEGIN{OFS="\t"}{print $0,namess}'  > /home/vincent/Projects/2019_pilotplant/05_mapping2DB/phageDBmapping/finalmapping/${names}_finalPhages_forR.bed
    

    done
cat /home/vincent/Projects/2019_pilotplant/05_mapping2DB/phageDBmapping/finalmapping/*.bed > /home/vincent/Projects/2019_pilotplant/05_mapping2DB/phageDBmapping/finalmapping/all_Phages_allSamples.txt

###-------------on local machine

scp vincent@130.223.51.151:/home/vincent/Projects/2019_pilotplant/05_mapping2DB/phageDBmapping/finalmapping/all_Phages_allSamples.txt /home/vincent/Desktop/Projects/2019_Pilotplan/09_annoation/all_Phages_allSamples.txt
...
#-------------file import
  ...
  read_count <- read_delim("/home/vincent/Desktop/Projects/2019_Pilotplan/09_annoation/all_Phages_allSamples.txt","\t", escape_double = FALSE, col_names = c("chr","start","end","name","unknown2","orientation","origin","sort","unknown","description","count","length_mapped","geneLength","sample2","sample"),trim_ws = TRUE)
...
phage_final <- read_count %>% 
  ...
  summarize(median = median(geneCoverage))
...
rm -r /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/bacteriaDBmapping/
...
echo ${num}"/16  :" ${names}
    ...
    rm -r /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/bacteriaDBmapping/${names}
    mkdir -p /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/bacteriaDBmapping/${names}
    

bedtools coverage -bed -a /archiv/Projects/2019_pilotplant/02_annotation/PGAP_for_Abundance/Bacteria_pgap_forAbundance.gff -b /archiv/Projects/2019_RMK202_analysis/03_mapping2Nanopre/02_againstpolished_single_K1_final_kneaddata/${names}/bwaMapping2DB/${names}_rawIllumina_bwamem_sorted.bam  > \
  /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/bacteriaDBmapping/${names}/${names}2bacteriaDB.bed 
...
names=Konserve_202
 #rm -r /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/phageDBmapping
 mkdir -p /home/vincent/Projects/2019_pilotplant/05_mapping2DB/bacteriaDBmapping/finalmapping/
for names in $(cat /archiv/Projects/2019_RMK202_analysis/01_log/names_onlymeta.txt)
    do
    
echo $name 
  
    cat /home/vincent/Projects/2019_pilotplant/05_mapping2RefAssembly/bacteriaDBmapping/${names}/${names}2bacteriaDB.bed | sed -e 's/ID=.*;OG=/OG=/g'|awk -F "[\t]" -v namess="$names"  'BEGIN{OFS="\t"}{print $0,namess}'  > /home/vincent/Projects/2019_pilotplant/05_mapping2DB/bacteriaDBmapping/finalmapping/${names}_finalBacteria_forR.bed
    

    done
cat /home/vincent/Projects/2019_pilotplant/05_mapping2DB/bacteriaDBmapping/finalmapping/*.bed > /home/vincent/Projects/2019_pilotplant/05_mapping2DB/bacteriaDBmapping/finalmapping/all_Bacteria_allSamples.txt

 cut -f 1 /home/vincent/Projects/2019_pilotplant/05_mapping2DB/bacteriaDBmapping/finalmapping/all_Bacteria_allSamples.txt | sort|uniq -c
 
###-------------on local machine

scp vincent@130.223.51.151:/home/vincent/Projects/2019_pilotplant/05_mapping2DB/bacteriaDBmapping/finalmapping/all_Bacteria_allSamples.txt /home/vincent/Desktop/Projects/2019_Pilotplan/09_annoation/all_Bacteria_allSamples.txt
...
#-------------file import
  ...
  read_count_bac <- read_delim("/home/vincent/Desktop/Projects/2019_Pilotplan/09_annoation/all_Bacteria_allSamples.txt","\t", escape_double = FALSE, col_names = c("chr","type","sort","start","end","unknown2","orientation","unknown","description","count","length_mapped","geneLength","covered","sample"),trim_ws = TRUE)
...
all_final$sample <- factor(all_final$sample, levels=(c("Lyo_202_2012","Lyo_202_2014","Konserve_202","RMK202","Versand_202")))
...
p2 <-  ggplot( data = all_final,aes(y = percent_coverage, x = sample, group=interaction(species),fill = species))+ geom_bar( stat="identity")+
    labs("",
         x="",
         y="relative abundance")+
    theme_classic()+
