...
##==================
colnames(sample_relative_wide)
sample_relative_long <- gather(sample_relative_wide, sample, Snps, "mst6":"cheesemaking\nday2", factor_key=TRUE,na.rm = TRUE) 
...
colnames(sample_relative_safe_final_prep_rarefy_wide) <- revalue(colnames(sample_relative_safe_final_prep_rarefy_wide), c("lyo202_96"="Lyo\n1996","Lyo_202_2012"="Lyo\n2012", "Lyo_202_2014"="Lyo\n2014", "Konserve_202"="working\nstock", "RMK202"="starter\nculture\n2012", "Versand_202"="starter\nculture\n2018","di_K2_6h"="cheesemaking\nday1","th_K2_8h"="cheesemaking\nday2"))
...
sample_relative_safe_woSTRAINS_sub01 <- sample_relative_safe_woSTRAINS
...
# # 
# dev.off()
  svg("~/Desktop/Manuscripts/2019_RMK202/Figures/F1_relativeAbundance_bac_snps_sterm.svg",width=4,height=1.8)
   # png("~/Desktop/Manuscripts/2019_RMK202/Figures/F1_relativeAbundance.png", width = 1000, height = 800,res=300)
...
sample_relative_long_Core <-  gather(sample_relative_wide, sample, Snps, "Lyo\n1996","Lyo\n2012","Lyo\n2014","working\nstock","starter\nculture\n2012","starter\nculture\n2018","cheesemaking\nday1","cheesemaking\nday2", factor_key=TRUE,na.rm = TRUE) %>% filter(Snps>0.05)   %>% group_by(sample,species) %>% tally()# %>% filter(core=="core" & Snps>0.05) 
...
clusterNames<- revalue(clusterNames,c("6"="4","5"="1","7"="2","8"="1","9"="1","4"="3"))##add certain clusters to others
...
str(non_phase_qunatiles$final_clust)
...
non_phase_qunatiles$final_clust = factor(non_phase_qunatiles$final_clust, levels=(sort(unique(non_phase_qunatiles$final_clust),decreasing = TRUE)))
plotColors <- colorsTSNE[as.numeric(levels(non_phase_qunatiles$final_clust))]
...
change_name <- setNames(1:length(decreasing_clusternames),decreasing_clusternames)
clusterNames <- revalue(as.factor(ds.real$cluster),change_name)
clusterNames<- revalue(clusterNames,c("4"="1","5"="4"))##add certain clusters to others
tsne_plot$density = factor(clusterNames)
ds.cent = tsne_plot %>% group_by(density) %>% select(V1, 
    V2) %>% summarize_all(mean)
ggplot(tsne_plot, aes(x = V1, y = V2, colour = density)) +
...
change_name <- setNames(1:length(decreasing_clusternames),decreasing_clusternames)
clusterNames <- revalue(as.factor(ds.real$cluster),change_name)
# clusterNames<- revalue(clusterNames,c("6"="5","5"="1","7"="2","8"="1","9"="1"))##add certain clusters to others
tsne_plot$density = factor(clusterNames)
ds.cent = tsne_plot %>% group_by(density) %>% select(V1, 
    V2) %>% summarize_all(mean)
ggplot(tsne_plot, aes(x = V1, y = V2, colour = density)) +
...
# explained_both_plot_nonModi_CLUSTERED$final_clust = factor(explained_both_plot_nonModi_CLUSTERED$final_clust, levels=(sort(unique(explained_both_plot_nonModi_CLUSTERED$final_clust),decreasing = TRUE)))
...
# non_phase_qunatiles$final_clust = factor(non_phase_qunatiles$final_clust, levels=c(2:1))
# plotColors <- colorsTSNE[as.numeric(levels(non_phase_qunatiles$final_clust))]
...
tsne_plot$density = factor(clusterNames)
tsne_plot$density <- revalue(tsne_plot$density,c("6"="1"))
ds.cent = tsne_plot %>% group_by(density) %>% select(V1, 
    V2) %>% summarize_all(mean)
ggplot(tsne_plot, aes(x = V1, y = V2, colour = density)) +
    geom_point(alpha = 0.3) + theme_bw() + geom_label_repel(aes(label = density),
...
explained_none_plot_nonModi_CLUSTERED$final_clust = factor(explained_none_plot_nonModi_CLUSTERED$final_clust, levels=(sort(unique(explained_none_plot_nonModi_CLUSTERED$final_clust),decreasing = TRUE)))
plotColors <- colorsTSNE[as.numeric(levels(explained_none_plot_nonModi_CLUSTERED$final_clust))]
...
##-----===Ribbon
  ...
  p2 <- (p_both_sterm_woModi_NOT_clustered+theme(axis.text.x = element_blank())|Sterm_both_suplementaryTsne|both_phased_ribbon+theme(axis.text.x = element_blank(),legend.position = "none")+annotate("text", label = "both strains", x = 5, y = 0.25, size = 7)|Sterm_both_suplementarybarPlot|Sterm_both_point+theme(axis.title.x = element_blank())|  plot_layout(widths = c(2,1,2, 1,1))) /
 (p_mst1_sterm_woModi_NOT_clustered+theme(axis.text.x = element_blank())|Sterm_mst1_suplementaryTsne|mst1_phased_ribbon+theme(axis.text.x = element_blank(),legend.position = "none")+annotate("text", label = "mst1", x = 5, y = 0.25, size = 7)|Sterm_mst1_suplementarybarPlot|Sterm_mst1_point+theme(axis.title.x = element_blank())| plot_layout(widths = c(2,1,2, 1,1))) /
 (p_mst2_sterm_woModi_NOT_clustered+theme(axis.text.x = element_blank())|Sterm_mst2_suplementaryTsne|mst2_phased_ribbon+theme(axis.text.x = element_blank(),legend.position = "none")+annotate("text", label = "mst2", x = 5, y = 0.75, size = 7)|Sterm_mst2_suplementarybarPlot|Sterm_mst2_point+theme(axis.title.x = element_blank())|  plot_layout(widths = c(2,1,2, 1,1))) /
 (p_none_sterm_woModi_NOT_clustered|Sterm_none_suplementaryTsne|none_phased_ribbon+theme(legend.position = "none")+annotate("text", label = "not explained", x = 5, y = 0.85, size = 7)|Sterm_none_suplementarybarPlot|Sterm_none_point|plot_layout(widths = c(2,1,2, 1,1)))
